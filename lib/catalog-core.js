...
function checkCatalog(dir, opts) {
  ...
  // file is valid JSON, only the BOM byte would trip JSON.parse.
  const load = f => { try { return flatten(JSON.parse(fs.readFileSync(f, 'utf8').replace(/^﻿/, ''))); } catch (e) { return { __error: String(e.message) }; } };
