...
function checkCatalog(dir, opts) {
  ...
  for (const loc of locales) {
    ...
    for (const [key, bval] of Object.entries(baseMap)) {
      ...
      // Inner-placeholder drift, for plural/select/selectordinal AND plain strings:
      // placeholders() ignores the ICU skeleton (comma-bearing `{count,plural…}`)
      // and category words, so it catches a dropped inner {name}/%s in any arm.
