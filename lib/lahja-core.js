...
const { detectScripts, anyNoSpaces, anyComplex } = require('./scripts');
...
function scanJs(src, ext, findings, lineOffset = 0) {
  ...
  babelTraverse(ast, {
    ...
    VariableDeclarator(p) {
      ...
      if (id && id.type === 'Identifier') {
        ...
        if (lit !== null && hasSegScript(lit)) segVars.add(id.name);
      }
  ...
  babelTraverse(ast, {
    CallExpression(p) {
      ...
      if (segTextOf(callee.object) === null) return;
