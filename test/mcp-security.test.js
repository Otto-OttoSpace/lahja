 ...
 * outside os.tmpdir(). See mcp/lahja-mcp.js.
 */
...
const MCP = path.join(__dirname, '..', 'mcp', 'lahja-mcp.js');
...
function driveCheckCode(ext, code) {
  return new Promise((resolve) => {
    ...
    send({ jsonrpc: '2.0', id: 2, method: 'tools/call', params: { name: 'lahja_check_code', arguments: { code, ext } } });
    setTimeout(done, 5000); // safety net
  });
}

test('check_code rejects a path-traversal ext (no arbitrary write/delete)', async () => {
  const victimDir = fs.mkdtempSync(path.join(os.tmpdir(), 'lahja-victim-'));
...
// ── regression: a flag-shaped scan `path` must not hijack the CLI ────────────
test('lahja_scan rejects a flag/subcommand-shaped path', async () => {
  const { spawn } = require('node:child_process');
  const drive = (p) => new Promise((resolve) => {
    ...
    s({ jsonrpc: '2.0', id: 2, method: 'tools/call', params: { name: 'lahja_scan', arguments: { path: p } } });
